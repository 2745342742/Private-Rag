...
def write_markdown_report(
    ...
    lines = []
    lines.append(f"# RAG 评估报告 — {report.get('project')}")
    lines.append("")
    if report.get("config_path"):
        lines.append(f"配置：`{report['config_path']}`  ")
    lines.append(f"样例数：{report.get('num_examples')}  ")
    lines.append(f"平均延迟：{report.get('avg_latency_ms'):.1f} ms  ")
